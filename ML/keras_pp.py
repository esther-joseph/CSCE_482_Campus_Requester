...
given_string = '10.95' #switch this to accept input from app here

def learn_string(string_price):


    train_score = model.evaluate(arr_x_train, arr_y_train, verbose=0)
    valid_score = model.evaluate(arr_x_valid, arr_y_valid, verbose=0)

    train_mae = round(train_score[1], 4)
    train_loss = round(train_score[0], 4)

    val_mae = round(valid_score[1], 4) 
    val_loss =  round(valid_score[0], 4)

    print('Train MAE: ', train_mae , ', Train Loss: ', round(train_score[0], 4)) 
    print('Val MAE: ',val_mae , ', Val Loss: ', val_loss)

    #transform string_price here with scores
    


    
    #modify and pull average from scores representing predictions



learn_string(given_string)
